...
import { saveChat } from "@/lib/mobile-storage";
...
export const useSendMessage = (chatId: string) => {
  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      // Persist final chat state to localStorage on mobile
      if (isNativeMobile()) {
        const finalData = queryClient.getQueryData<{ chat: Chat }>([
          "chat",
          currentChatId,
        ]);
        if (finalData?.chat) {
          saveChat(finalData.chat);
        }
      }
    },
  });
