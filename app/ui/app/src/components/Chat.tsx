...
export default function Chat({ chatId }: { chatId: string }) {
  ...
  return chatId === "new" || chatQuery ? (
    ...
    >
      {chatId === "new" ? (
        <div className="flex flex-col h-[100dvh] justify-center relative">
        ...
        <main className="flex h-[100dvh] w-full flex-col relative allow-context-menu select-none">
          <section
            key={chatId} // This key forces React to recreate the element when chatId changes
            ref={containerRef}
            className={`flex-1 overflow-y-auto overscroll-contain relative min-h-0 select-none pt-2 md:pt-0 ${isWindows ? "xl:pt-4" : "xl:pt-8"}`}
          >
            <MessageList
              messages={messages}
